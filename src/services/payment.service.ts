...
export class PaymentService {
  ...
  createPayment(customer: Customer, createPaymentForm: CreatePaymentForm): Observable<Payment> {
    return this.httpClient.post<PaymentData>(`/api/customers/${customer.id}/payments`, createPaymentForm).pipe(
      ...
      map(([paymentData, customerEntities]) => Payment.fromData(paymentData, customerEntities[paymentData.customerId])),
      // TODO sorting
  ...
  loadPayments(): Observable<Payment[]> {
    return this.httpClient.get<{ payments: PaymentData[] }>('/api/payments').pipe(
      ...
      withLatestFrom(this.store.select(fromStore.getCustomersEntities)),
      map(([paymentDatas, customerEntities]) => paymentDatas.map(data => Payment.fromData(data, customerEntities[data.customerId]))),
